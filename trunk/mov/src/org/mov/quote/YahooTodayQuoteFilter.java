...
public class YahooTodayQuoteFilter implements QuoteFilter {
     ...
     * Creates an instance of the filter.
     */
    public YahooTodayQuoteFilter(Symbol symbol) {
        this.symbol = symbol;
    }
...
//	      fields[0]"600058.SS"//symbol
//        fields[1]11.17//close
//        fields[2]"9/14/2004"//date
//        fields[3]"1:18am"//time
//        fields[4]+0.27//change
//        fields[5]10.90//open
//        fields[6]11.29//high
//        fields[7]10.90//low
//        fields[8]412520//volume
	    
	           
	    
	    if(quoteParts.length == 9) {//bryan
	        	        
		TradingDate date = null;

                try {
                    date = new TradingDate(quoteParts[2].substring(1, quoteParts[2].length()-1 ),
                            TradingDate.US);//bryan
                }
                catch(TradingDateFormatException e) {
                    System.out.println("TradingDateFormatException:"+e);
                    return null;
                    // Return null - couldn't parse quote
                }
                try {
                    float day_close = Float.parseFloat(quoteParts[1]);
                    float day_open = Float.parseFloat(quoteParts[5]);
                    float day_high = Float.parseFloat(quoteParts[6]);
                    float day_low = Float.parseFloat(quoteParts[7]);
                    int day_volume = Integer.parseInt(quoteParts[8]);
                    // the remaining one is adjusted day close
//System.out.println("O:"+day_open+"L:"+day_low);
