...
public class EMAExpression extends QuaternaryExpression {
    ...
    public EMAExpression(Expression quote, Expression days,
			 Expression smoothing, Expression lag) {
        super(quote, days, smoothing, lag);
    }
	...
	int period = (int)getChild(1).evaluate(variables, quoteBundle, symbol, day);
        ...
        double smoothing = (double)getChild(2).evaluate(variables, quoteBundle, symbol, day);
	...
	// First type must be quote, second and fourth types must be integer value,
        // the third (the smoothing constant) must be a float value
